...
/// Supports Cloudflare Email Service API and generic HTTP endpoints, with Log fallback.
#[derive(Clone)]
pub struct Mailer {
    inner: MailerKind,
        ...
        match &self.inner {
            MailerKind::Log => {
                tracing::info!(
                    to,
                    code,
                    "verification code (mail credentials not configured)"
                );
                Ok(())
            }
        ...
        match &self.inner {
            MailerKind::Log => {
                tracing::info!(
                    to,
                    verify_url,
                    "verify email (mail credentials not configured)"
                );
                Ok(())
            }
            MailerKind::Cloudflare {
