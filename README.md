...
Extracted from real-world, high-performance gateway and mesh architectures (**ParaTensor / xrouter**), this repository provides an out-of-the-box foundation for building robust backend services, high-signal admin consoles, and self-regulating AI agent workflows.
...
- **Token-Efficient Skills System**: Domain workflows, local gates, and safety procedures separated into `.agents/skills/` (read on demand).
...
- **Dense Admin Console**: React 19 + TypeScript + Tailwind + Radix UI with 100% symmetric i18n (English/Chinese).
...
| **`src/` & `admin/`** | **Starter Codebase**: Rust 2024 Axum backend baseline and React 19 + Tailwind admin console. | New Projects |
...
# Admin frontend quality gate (if applicable)
