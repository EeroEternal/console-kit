...
5. **UI Viewport & Layout Bounds**: Dialog popups are strictly bounded to `max-h-[85vh]` with internal `overflow-y-auto`; long content must use accordions or tabs, never tiling over the screen. Global configurations belong exclusively in the Settings page.
